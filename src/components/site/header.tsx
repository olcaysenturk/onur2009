      ...
      <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-4 md:h-20 md:px-10">
        ...
        <div className="hidden items-center gap-6 md:flex">
          <div className="group relative">
            ...
            <div className="invisible absolute right-0 top-full mt-3 w-44 rounded border border-outline/30 bg-surface py-2 opacity-0 shadow-xl transition-all group-hover:visible group-hover:opacity-100">
              {languageOptions.map(({ code, label, flag }) => (
                <button
                  type="button"
                  key={code}
                  onClick={() => changeLanguage(code)}
                  className="flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-muted transition-colors hover:bg-primary hover:text-white"
                >
          ...
          <div className="mt-5 flex flex-wrap gap-2 border-t border-outline/30 pt-5">
            {languageOptions.map(({ code, label, flag }) => (
              <button
                ...
                type="button"
                onClick={() => changeLanguage(code)}
                className="inline-flex items-center gap-2 rounded border border-outline/40 px-3 py-2 text-sm text-muted"
              >
                <span aria-hidden>{flag}</span>
                {label}
              </button>
